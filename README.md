...
│         / Sparkplug B                             │
├──────────────────────────────────────────────────┤
│  Processing 处理层（边缘计算: 过滤/聚合 + 告警引擎）│
├──────────────────────────────────────────────────┤
...
数据流:`scheduler 采集 ──DataPoint──▶ channel ──▶ 处理层(过滤/聚合) ──▶ 告警引擎 ──▶ 输出(数据) + 告警(定向投递)`
...
采集数据在边缘即判断告警:规则用表达式引用任意设备点位的**最新值**,求值为 `true` 即**边沿触发**
一次告警(条件解除自动恢复),写入 SQLite 告警表并**定向投递**到规则指定的输出(目前 MQTT 支持
告警 topic)。配置经 Web UI「**告警规则**」页完成(引用点位从已配置设备点位中选择、逐点位填比较条件、
选投递输出),**保存即热重载**;触发记录在「**告警记录**」页按状态(未解除/已解除)查看。
...
- 引用任意设备点位最新值,支持算术 / 比较 / 逻辑(`&&`、`||`、括号);
- 每个引用点须为**可求值为 true/false 的比较条件**(如 `> 30`、`== "off"`),空条件不支持;
- **新鲜度**(默认 300s):任一引用点超期未更新则跳过本轮判断,防陈旧值误报;
- **防抖**(默认 0s):解除后窗口内抑制重触发,防告警抖动风暴;
- 告警消息默认发到 MQTT topic `gateway/{网关ID}/alert`(可用输出配置 `alertTopic` 覆盖),
  含触发瞬间各引用点值快照(`context`)。

完整使用说明(字段详解 / 表达式语法 / 触发行为 / 消息格式 / REST / 排障)见
[docs/alert-rules.md](docs/alert-rules.md),设计见 [docs/alert-engine-design.md](docs/alert-engine-design.md)。
...
连接管理、设备管理(点位/克隆/写值)、**北向输出配置**、**告警规则与告警记录**、设备/输出运行状态与边缘处理统计。
