...
同步创建/更新的连接与设备在自身行上带 `managed_by` 标记（`smardaten:<outputID>`，按输出实例隔离），
孤儿集合 = 带本输出实例标记、但本次配置中已不存在的实体，直接按行查询即可，**无需任何登记表/JSON 持久化**。
天然区分"平台同步创建"与"Web UI 手工配置"（后者 `managed_by` 为空）——**只清理前者，不误删本地手工配置**。
删除顺序先设备后连接；连接仍被设备引用时删除失败（`ErrConnectionInUse`），孤儿行仍带标记，引用解除后下次同步自动重试。
...
6. **自动同步的覆盖语义**：平台下发配置时会 upsert 覆盖同名 Connection/Device。若网关本地设备与平台设备 ID 冲突，会被平台配置覆盖（预期行为，平台为权威源）。若需保留本地设备，需引入"本地优先"标志或命名空间隔离。**✅ 平台移除的实体（孤儿）已实现自动删除**：仅删平台同步创建、本次配置中已不存在的实体，管理集合以实体行的 `managed_by` 标记表达（按输出实例隔离，`smardaten:<outputID>`），不误删 Web UI 手工配置（见 §8.5）。
