...
func (*modbusDriver) ConfigSchema() []driver.Field {
	return []driver.Field{
		...
		{Name: "address", Label: "地址", Type: driver.FieldString, Placeholder: "192.168.1.5:502",
			Hint: "tcp / rtu-over-tcp 必填", ShowWhen: &driver.ShowWhen{Field: "mode", In: []string{"tcp", "rtu-over-tcp"}}},
		{Name: "serialPort", Label: "串口", Type: driver.FieldString, Placeholder: "/dev/ttyS0",
			ShowWhen: &driver.ShowWhen{Field: "mode", In: []string{"rtu"}}},
...
// EndpointKey 归一化物理端点:key 不含模式前缀,同一 host:port 无论 tcp 还是
// rtu-over-tcp 访问(后者即 DTU 透传的 485 总线),都算同一端点,并存即并发写
// 同一条总线;rtu 按串口路径(串口只能被一个连接独占)。
func (*modbusDriver) EndpointKey(connection json.RawMessage) string {
	...
	case "tcp", "rtu-over-tcp":
		return "tcp|" + strings.ToLower(strings.TrimSpace(cfg.Address))
	case "rtu":
		return "serial|" + strings.ToLower(strings.TrimSpace(cfg.SerialPort))
	default:
		return ""
	}
}
